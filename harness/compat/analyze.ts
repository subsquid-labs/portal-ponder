...
const isFactory = (addr: any): boolean =>
  ...
  typeof addr === "object" &&
  !Array.isArray(addr) &&
  ("event" in addr || "parameter" in addr || "childAddressLocation" in addr);

function chainEntries(src: any): { chain: string; override: any }[] {
  if (typeof src.chain === "string")
    return [{ chain: src.chain, override: src }];
  if (src.chain && typeof src.chain === "object")
    return Object.entries(src.chain).map(([chain, override]) => ({
      chain,
      override: { ...src, ...(override as any) },
    }));
  return [];
}
...
const startBlockOf = (override: any): number | undefined => {
  ...
  if (
    isFactory(override.address) &&
    typeof override.address.startBlock === "number"
  )
    return override.address.startBlock;
  return undefined;
};

export function analyzeConfig(
  config: any,
  catalog: Map<string, DatasetInfo> = new Map(),
): CompatReport {
  ...
  const resolve = (
    cid: number,
  ): { dataset: string | null; served: boolean | null; caps?: NetworkCaps } => {
  ...
  const push = (
    ...
    if (!dataset) {
      verdict = "NO_DATASET";
      blockers.push(
        "no SQD Portal dataset for this chain (not in the docs network matrix)",
      );
    } else if (served === false) {
      verdict = "NO_DATASET";
      blockers.push(
        `this portal does not serve '${dataset}' (per its /datasets); a different portal may`,
      );
    } else {
      for (const n of needsArr)
        if (SUPPORT[n] === "todo")
          blockers.push(`${n} not yet implemented in the Portal backfill`);
      if (needs.has("traces") && caps && !caps.traces)
        blockers.push(`Portal has no traces for ${dataset}`);
      if (caps?.note && (needs.has("traces") || needs.has("transactions")))
        notes.push(caps.note); // e.g. "traces from Bedrock block …"
      if (blockers.length === 0) verdict = "READY";
      else if (blockers.some((b) => b.includes("trace")))
        verdict = "NEEDS_TRACES";
      else if (needsArr.includes("receipts")) verdict = "NEEDS_RECEIPTS";
      else if (needsArr.includes("blockInterval"))
        verdict = "NEEDS_BLOCK_FILTER";
    ...
    sources.push({
      ...
      chainId: cid,
      dataset,
      datasetRealTime: caps?.realtime ?? null,
      startBlock,
      needs: needsArr,
      verdict,
      blockers,
  ...
  for (const [name, c] of Object.entries(config.contracts ?? {}))
    for (const { chain, override } of chainEntries(c))
      push(name, chain, ["logs", "transactions"], override);
  for (const [name, a] of Object.entries(config.accounts ?? {}))
    for (const { chain, override } of chainEntries(a))
      push(`account:${name}`, chain, ["transactions", "accountTx"], override);
  for (const [name, b] of Object.entries(config.blocks ?? {}))
    for (const { chain, override } of chainEntries(b))
      push(`block:${name}`, chain, ["blockInterval"], override);
  ...
  const chains: ChainSummary[] = Object.entries(config.chains ?? {}).map(
    ([name, c]: any) => {
