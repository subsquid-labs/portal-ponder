...
export type BenchResult = {
  name: string; ok: boolean; wallSec: number; events: number; eventsPerSec: number;
  peakRssMB: number; error?: string;
  portal: {
    http: number; mb: number; dataChunks: number; errors: number; retries: number;
    cacheHits: number; maxInflight: number; rpcFallback: number; chunkBlocks: number; inserted: Record<string, number>;
  } | null;
};
...
export async function runBench(spec: BenchSpec): Promise<BenchResult> {
  ...
  for (const cid of spec.chainIds) try { rmSync(`${metricsBase}.${cid}`); } catch {}
  try { rmSync(join(spec.dir, ".ponder"), { recursive: true, force: true }); } catch {}

  const env: Record<string, string> = {
    ...(process.env as Record<string, string>),
    PONDER_START: String(spec.start), PONDER_END: String(spec.end),
  ...
  const proc = spawn(bin, ["start", "--schema", spec.schema, "--port", String(spec.port)], { cwd: spec.dir, env });

  const t0 = Date.now();
  let events = 0, peakRssMB = 0, done = false, failed = "";
  let buf = "";
  const onLine = (line: string) => {
    const m = line.match(/event_count=(\d+)/); if (m) events += Number(m[1]);
    if (/Completed indexing across all chains/.test(line)) done = true;
    if (/heap out of memory|FATAL|Error while processing|BuildError|Cannot find/i.test(line)) failed ||= strip(line).slice(0, 180);
  };
  const onData = (d: Buffer) => { buf += d.toString(); let nl: number; while ((nl = buf.indexOf("\n")) >= 0) { onLine(buf.slice(0, nl)); buf = buf.slice(nl + 1); } };
  ...
  if (!done && !failed) failed = proc.exitCode !== null ? `process exited (code ${proc.exitCode})` : "timeout";
  try { proc.kill("SIGTERM"); } catch {}
  await sleep(400); try { proc.kill("SIGKILL"); } catch {}

  let portal: BenchResult["portal"] = null;
  for (const cid of spec.chainIds) {
    const file = `${metricsBase}.${cid}`;
    if (!existsSync(file)) continue;
    let m: any; try { m = JSON.parse(readFileSync(file, "utf8")); } catch { continue; }
    portal ??= { http: 0, mb: 0, dataChunks: 0, errors: 0, retries: 0, cacheHits: 0, maxInflight: 0, rpcFallback: 0, chunkBlocks: 0, inserted: {} };
    portal.http += m.fetch.http; portal.mb += m.fetch.bytes / 1e6; portal.dataChunks += m.fetch.dataChunks;
    portal.errors += m.fetch.errors; portal.retries += m.fetch.retries; portal.cacheHits += m.fetch.cacheHits;
    portal.maxInflight = Math.max(portal.maxInflight, m.fetch.maxInflight); portal.rpcFallback += m.rpcFallbackIntervals;
    portal.chunkBlocks = m.chunkBlocks;
    for (const k of Object.keys(m.inserted)) portal.inserted[k] = (portal.inserted[k] ?? 0) + m.inserted[k];
  }

  const ok = done && !failed;
  return {
    name: spec.name, ok, wallSec: +wallSec.toFixed(1), events,
    eventsPerSec: wallSec > 0 ? Math.round(events / wallSec) : 0,
    peakRssMB: Math.round(peakRssMB), error: failed || undefined, portal,
  };
}
