...
const specs = BENCHES.filter(
  (b) => !filter || filter.split(',').some((f) => b.name.includes(f.trim())),
);
if (specs.length === 0) {
  console.error(
    `no benches match '${filter}'. available: ${BENCHES.map((b) => b.name).join(', ')}`,
  );
  process.exit(1);
}
...
for (const spec of specs) {
  ...
  results.push(r);
  process.stderr.write(
    `  ${r.ok ? '✅' : '❌ ' + r.error}  ${r.wallSec}s  ${r.events.toLocaleString()} events  ${r.eventsPerSec.toLocaleString()}/s  rss ${r.peakRssMB}MB` +
      (r.portal
        ? `  | portal: http ${r.portal.http} ${r.portal.mb.toFixed(1)}MB chunks ${r.portal.dataChunks} err ${r.portal.errors} retry ${r.portal.retries} fallback ${r.portal.rpcFallback}`
        : '') +
      '\n',
  );
}
...
const line = (cells: (string | number)[]) =>
  cells.map((c, i) => (i <= 1 ? pad(c, W[i]) : padL(c, W[i]))).join(' ');
const rows = results.map((r) =>
  line([
    ...
    r.peakRssMB,
    r.portal?.http ?? '-',
    r.portal ? r.portal.mb.toFixed(1) : '-',
    r.portal?.dataChunks ?? '-',
    r.portal?.errors ?? '-',
    r.portal?.retries ?? '-',
    r.portal?.rpcFallback ?? '-',
  ]),
);
const table = [
  line(H),
