#!/usr/bin/env bash
# kill-controller.sh - Phase A realtime chaos driver (foundation + K1 proof).
#
# Runs:
#   1. Build disposable @subsquid/ponder 0.16.9 graft.
#   2. Install the Euler stream app against it.
#   3. Build an unkilled K1 baseline, releasing the K1 phase gate.
#   4. Kill at K1-append, reset the mock, resume, release the gate, verify digests.
set -uo pipefail
...
SCENARIO="${SCENARIO:-$CDIR/scenarios/k1-append.json}"
TARGET_PHASE="${TARGET_PHASE:-K1-append}"
...
RUN_ROOT="$CDIR/run/phaseA-$(date -u +%Y%m%dT%H%M%SZ)-$$"

mkdir -p "$CDIR/work"
WORK="$(mktemp -d "$CDIR/work/phaseA.XXXXXX")"
...
wait_complete () {
  ...
  while [ "$i" -lt 1800 ]; do
    if invariant "$log_file"; then
      log "fatal: InvariantViolation in $log_file"
      return 2
    fi
...
ensure_graft () {
  ...
  ( cd "$ROOT" && SYNC_WORKDIR="$graft" scripts/sync-upstream.sh "$PONDER_VERSION" ) \
    >"$RUN_ROOT/sync-upstream.log" 2>&1 || {
      ...
      tail -80 "$RUN_ROOT/sync-upstream.log" || true
      return 1
    }
...
NODE
  ...
  ( cd "$app" && npm install --no-audit --no-fund --silent --cache "$NPM_CACHE" ) \
    >"$RUN_ROOT/npm-install.log" 2>&1 || {
      ...
      tail -80 "$RUN_ROOT/npm-install.log" || true
      return 1
    }
  echo "$app"
}
...
start_app () {
  ...
  APP_PID="$!"
  APP_PGID="$APP_PID"
  log "started app pid=$APP_PID pgid=$APP_PGID log=$log_file"
}
...
run_kill_resume () {
  local app="$1"
  local db="rg3_phasea_k1_$$"
  local url="postgres://postgres@127.0.0.1:$PGPORT/$db"
  local dir="$RUN_ROOT/k1"
  mkdir -p "$dir"
  drop_create_db "$db" || return 1
  start_mock "$dir/phase.log" "$dir/mock.log" || return 1
  start_app "$app" "$url" "$dir/kill.app.log"
  wait_phase "$TARGET_PHASE" || {
    log "fatal: kill run did not reach $TARGET_PHASE"
    return 1
  }
  cp "$dir/phase.log" "$dir/kill.phase.log"
  kill_app
  if invariant "$dir/kill.app.log"; then
    log "fatal: InvariantViolation before kill"
    return 1
  fi
  ...
  wait_phase "$TARGET_PHASE" || {
    log "fatal: resume run did not reach $TARGET_PHASE"
    return 1
  }
  curl_json POST /__release "{\"phase\":\"$TARGET_PHASE\"}" >/dev/null || return 1
  wait_complete "$APP_PID" "$dir/resume.app.log" || return 1
  kill_app
  curl_json GET /__stats > "$dir/mock-stats.json" || true
  stop_mock

  ROOT="$ROOT" PROBE_DIR="$app" STORE_SCHEMA="$STORE_SCHEMA" APP_SCHEMA="$APP_SCHEMA" \
    "$VERIFY" "$url" "$BASE_DIR/k1-append.store.digest" "$BASE_DIR/k1-append.app.digest" \
      "$dir/kill.phase.log" "$TARGET_PHASE" >"$dir/verify.log" 2>&1 || {
...
main () {
  ...
  CHAOS_WORK="$WORK_ROOT" CHAOS_PGPORT="$PGPORT" CHAOS_PGSOCK="$PGSOCK" bash "$PGCTL" ensure \
    >"$RUN_ROOT/pg-ctl.log" 2>&1 || {
      ...
      tail -80 "$RUN_ROOT/pg-ctl.log" || true
      exit 1
    }
  local fsync
  ...
  run_baseline "$app" || exit 1
  run_kill_resume "$app" || exit 1
  log "Phase A K1 proof complete; artifacts: $RUN_ROOT"
}
