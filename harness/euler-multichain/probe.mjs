...
const targets = chains
  .filter((c) => REALTIME_CHAINS.has(c.id))
  .map((c) => ({
    chainId: c.id,
    chain: c.name,
    endpoints: [
...
async function tick() {
  ...
  for (const s of samples)
    if (s.ok && s.tip != null)
      maxTip[s.chainId] = Math.max(maxTip[s.chainId] ?? 0, s.tip);
  for (const s of samples)
    s.lag =
      s.ok && s.tip != null && maxTip[s.chainId] != null
        ? maxTip[s.chainId] - s.tip
        : null;
  ring.push(...samples);
  ...
  for (const [k, ss] of Object.entries(byKey)) {
    ...
    summary[k] = {
      ...
      endpoint: ss[0].name,
      n: ss.length,
      okRate: +(ok.length / ss.length).toFixed(3),
      latP50: pct(lats, 50),
      latP95: pct(lats, 95),
      latMean: lats.length
        ? Math.round(lats.reduce((a, b) => a + b, 0) / lats.length)
        : 0,
      avgLagBlk: lags.length
...
console.log(
  `probe: ${targets.length} chains × endpoints, interval ${INTERVAL}ms -> ${METRICS}`,
);
tick();
