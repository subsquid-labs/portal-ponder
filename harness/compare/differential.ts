...
const ADDRESS = (process.env.ADDRESS ?? "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2").toLowerCase();
const TOPIC0 = process.env.TOPIC0 ?? "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef";
const FROM = Number(process.env.FROM ?? 21_000_000);
const TO = Number(process.env.TO ?? 21_000_050);

const key = (l: { blockNumber: string; logIndex: string }) => `${BigInt(l.blockNumber)}:${BigInt(l.logIndex)}`;

// --- Portal path ---
const portal = new PortalClient({ dataset: DATASET });
const portalLogs: ReturnType<typeof toSyncLog>[] = [];
for await (const batch of portal.streamFinalized(buildPortalQuery([FROM, TO], [{ address: [ADDRESS], topic0: [TOPIC0] }]))) {
  for (const b of batch.blocks) for (const log of b.logs ?? []) portalLogs.push(toSyncLog(log, b.header));
}
...
const rpcLogs = await rpc.request({
  method: "eth_getLogs",
  params: [{ address: ADDRESS as `0x${string}`, topics: [TOPIC0 as `0x${string}`], fromBlock: numberToHex(FROM), toBlock: numberToHex(TO) }],
}) as any[];

// --- compare ---
const portalMap = new Map(portalLogs.map((l) => [key(l), l]));
const rpcMap = new Map(rpcLogs.map((l) => [key({ blockNumber: l.blockNumber, logIndex: l.logIndex }), l]));
...
for (const k of allKeys) {
  const p = portalMap.get(k), r = rpcMap.get(k);
  if (!p) { console.log(`  MISSING in Portal: ${k}`); mismatches++; continue; }
  if (!r) { console.log(`  EXTRA in Portal:   ${k}`); mismatches++; continue; }
  const fields: [string, unknown, unknown][] = [
    ...
    ["blockNumber", BigInt(p.blockNumber).toString(), BigInt(r.blockNumber).toString()],
    ["logIndex", BigInt(p.logIndex).toString(), BigInt(r.logIndex).toString()],
    ["txIndex", BigInt(p.transactionIndex).toString(), BigInt(r.transactionIndex).toString()],
  ];
  for (const [name, a, b] of fields) {
    if (a !== b) { console.log(`  FIELD MISMATCH ${k} ${name}: portal=${a} rpc=${b}`); mismatches++; }
  }
}
