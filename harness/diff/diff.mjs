...
// stores event-less blocks it traced (inert, never referenced) — reported, not failed.
{
  ...
  const [a, b] = await Promise.all([
    dump(dirA, "blocks", BLOCK_DROP).then(byHash),
    dump(dirB, "blocks", BLOCK_DROP).then(byHash),
  ]);
  ...
  console.log(
    `  ${ok ? "✅" : "❌"} ${"blocks".padEnd(20)} portal=${String(a.size).padStart(6)}  rpc=${String(b.size).padStart(6)}  shared=${shared.length} match` +
      (rpcExtra ? `, +${rpcExtra} inert event-less (RPC-only)` : "") +
      (ok ? "" : `  | ${mismatch.length} shared MISMATCH`),
  );
  if (!ok) {
    fail = 1;
    for (const h of mismatch.slice(0, 2)) {
      console.log("       portal:", a.get(h).slice(0, 320));
      console.log("       rpc:   ", b.get(h).slice(0, 320));
    }
  }
}
