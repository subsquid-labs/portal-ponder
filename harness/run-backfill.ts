...
console.log(`range [${START.toLocaleString()}, ${TO.toLocaleString()}] = ${(TO - START).toLocaleString()} blocks | finalizedHead=${head.number}`);
console.log(`sources: ${chain.factories.map((f) => f.name).join(", ")}${chain.singletons.length ? " + " + chain.singletons.map((s) => s.name).join(",") : ""}`);
...
for (const f of chain.factories) console.log(`  ${f.name}: ${children.get(f.name)!.size} children (${p1http} http, ${p1s}s)`);
...
for (const f of chain.factories) {
  ...
  if (addrs.length && f.childTopic0s.length) dataFilters.push({ address: addrs, topic0: f.childTopic0s });
}
for (const s of chain.singletons) if (s.topic0s.length) dataFilters.push({ address: [s.address], topic0: s.topic0s });

const topicLabel = new Map<string, string>();
for (const f of chain.factories) f.childTopic0s.forEach((t, i) => topicLabel.set(t, `${f.name}.${f.childEventNames[i]}`));
for (const s of chain.singletons) s.topic0s.forEach((t, i) => topicLabel.set(t, `${s.name}.${s.eventNames[i]}`));

const perEvent = new Map<string, number>();
let sampleRow: unknown;
const t2 = Date.now();
let p2http = 0, totalLogs = 0;
for await (const batch of client.streamFinalized(buildPortalQuery([START, TO], dataFilters))) {
  p2http++;
  for (const b of batch.blocks) {
    for (const log of b.logs ?? []) {
      totalLogs++;
  ...
  if (p2http % 25 === 0) console.log(`  …http#${p2http} block ${batch.toBlock} | ${totalLogs.toLocaleString()} logs`);
}
const p2s = ((Date.now() - t2) / 1000).toFixed(1);
console.log(`  ${totalLogs.toLocaleString()} child/singleton logs in ${p2http} http, ${p2s}s`);

// ---------- report ----------
console.log(`\n=== EVENTS INDEXED (per source) ===`);
for (const [label, n] of [...perEvent.entries()].sort((a, b) => b[1] - a[1])) console.log(`  ${label.padEnd(28)} ${n.toLocaleString()}`);
...
for (const [k, g] of Object.entries(gate)) {
  const ok = ("max" in g ? (g as any).value <= (g as any).max : true) && ("min" in g ? (g as any).value >= (g as any).min : true);
  pass &&= ok;
  console.log(`  [${ok ? "PASS" : "FAIL"}] ${k} = ${(g as any).value} (${"max" in g ? "≤" + (g as any).max : "≥" + (g as any).min})`);
}
console.log(`\n  GATE: ${pass ? "✅ PASS" : "❌ FAIL"}`);

const out = `/Users/dz/Projects/portal-ponder/harness/results.${chain.dataset}.json`;
writeFileSync(out, JSON.stringify({ chain: chain.chainId, dataset: chain.dataset, range: [START, TO], perEvent: Object.fromEntries(perEvent), metrics: snap, gate, pass }, null, 2));
console.log(`\nwrote ${out}`);
