...
The fork changes the **transport** — the SQD Portal instead of per-chain JSON-RPC — not the **data**: the rows it writes to the sync store match what stock Ponder writes from RPC, byte-identical wherever the Portal dataset is complete, with the rare upstream column gaps surfaced as honest `NULL` rather than fabricated (below). The evidence, consolidated in [`VALIDATION.md`](VALIDATION.md):

- **Byte-diff vs stock RPC.** A validation matrix backfills the same ranges two ways — Portal and genuine `ponder` over metered RPC (ground truth) — and diffs every sync-store row. Logs, transactions, and receipts come out byte-identical across chain families: full Euler history on Ethereum ([VALIDATION.md §3.2](VALIDATION.md)), plus receipts byte-identity on Polygon ([§3.6](VALIDATION.md)) and BSC ([§3.7](VALIDATION.md)).
- **Whole-store count parity.** The flagship 15-chain store matches the frozen **internal** reference store exactly — **62/62 aggregate cells, 0 diffs** ([VALIDATION.md §5.7](VALIDATION.md)).
...
- **Reorg & finality.** The Portal serves only finalized data for backfill, so historical rows are write-once (reorg-free by construction); realtime reorg reconciliation is mutation-tested. A reorg deeper than the finalized floor is a deliberate fail-loud limit — the indexer stops for an operator re-sync rather than silently diverging ([VALIDATION.md §5.8](VALIDATION.md)).
...
The residual Portal-vs-RPC differences are two bounded, documented **upstream-dataset** quirks: a dropped `access_list` column on some chains, surfaced as honest SQL `NULL` rather than fabricated data ([§5.6](VALIDATION.md)); and a single non-consensus `block.size` off-by-one, tolerated as benign. Neither is fork-introduced data loss — the fork's earlier fabricated-`[]` defect was itself caught by this matrix, fixed, and proven FAIL→PASS ([§5.6](VALIDATION.md), issue [#32](https://github.com/subsquid-labs/portal-ponder/issues/32) closed).
...
`@subsquid/ponder@X.Y.Z-sqd.<rev>` is `ponder@X.Y.Z` plus the Portal layer, pinned to a known Ponder version; `-sqd.<rev>` ships a fork-side fix on the same Ponder version. The seam is stable across 0.15.17–0.17.8.

| Ponder | `@subsquid/ponder` | Status |
|---|---|---|
| 0.17.8 | `0.17.8-sqd.1` | **latest** |
| 0.17.6 | `0.17.6-sqd.1` | published · pin exact (no dist-tag) |
| 0.17.4 | `0.17.4-sqd.1` | published · pin exact (no dist-tag) |
| 0.17.3 | `0.17.3-sqd.1` | published · pin exact (no dist-tag) |
| 0.17.2 | `0.17.2-sqd.1` | published · pin exact (no dist-tag) |
