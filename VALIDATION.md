...
`0.16.8`, `0.16.9`, `0.16.10`, `0.17.0`, `0.17.1`, and `0.17.2` (`compat.tested` in `versions.json`) — by grafting the
...
scripts/sync-upstream.sh 0.17.2  --test
```
...
- `0.17.2` ([#200](../../pull/200)) applies the `0.17.1` wiring patch **verbatim** (a sha256-equal copy,
  same 12 files); `0.17.1 → 0.17.2` is two upstream **patch-level bugfixes**, both off-seam or
  non-overlapping so the fork passively inherits them: (a)
  [`ponder-sh/ponder#2341`](https://github.com/ponder-sh/ponder/pull/2341) fixes rows returned by
  `context.db.find()`/`insert()`/`update()` being mutated by a subsequent write to the same row —
  implemented purely in `src/indexing-store/index.ts`, **off** the graft surface (like the `0.16.9`
  bytea-COPY fix); and (b)
  [`ponder-sh/ponder#2347`](https://github.com/ponder-sh/ponder/pull/2347) fixes realtime-discovered
  factory child addresses being omitted from `ponder_sync.factory_addresses` when a factory is shared
  across multiple event handlers — implemented in `src/sync-realtime/index.ts`, one of the 12 wiring
...
- `0.17.4` ([#213](../../pull/213)) applies the `0.17.3` wiring patch **verbatim** (a byte-identical copy,
  same 12 files); `0.17.3 → 0.17.4` is four off-seam upstream indexing-performance PATCH PRs (all
  [@kyscott18](https://github.com/kyscott18)):
  [`ponder-sh/ponder#2353`](https://github.com/ponder-sh/ponder/pull/2353) (batch internal metrics updates),
  [`#2356`](https://github.com/ponder-sh/ponder/pull/2356) (schema-aware copying),
  [`#2355`](https://github.com/ponder-sh/ponder/pull/2355) (normalize only changed row fields on updates),
  and [`#2352`](https://github.com/ponder-sh/ponder/pull/2352) (reduce copy-on-write row memory) — every one
  confined to ponder's **indexing** plane (`indexing-store/*`, `internal/metrics.ts`, `utils/copy.ts`,
  `indexing/index.ts`, and a one-line `flushIndexingMetrics()` add at the tail of each
  `runtime/{isolated,multichain,omnichain}.ts` indexing loop). Three of the changed files are wiring files,
  but each has a single wiring hunk at its `run*` entrypoint, above and non-overlapping the perf additions, so
  the patch applies with zero rejects. The Portal layer references none of the three changed metrics symbols
  (grep-verified across `portal/`); the graft seam (`HistoricalSync.syncBlockRangeData` / `syncBlockData`) is
  untouched.

The full current Portal suite is **416 tests across 24 files**, green on every tracked version via the
one-command gate (verified in the `0.16.10` / `0.17.0` / `0.17.1` / `0.17.2` / `0.17.3` / `0.17.4` / `0.17.6` / `0.17.8` compat work). As with `0.15.17`, this basis is
...
fork is now `@subsquid/ponder@0.17.8-sqd.1` (published 2026-08-18 with `--tag latest`, superseding
`0.17.6-sqd.1`, which now installs by exact version, no dist-tag; `0.16.10-sqd.1` remains under dist-tag
`ponder-0.16.10`, `versions.json`). `0.17.8-sqd.1` grafts the identical Portal layer onto ponder@0.17.8;
`wiring/0.17.8.patch` is a **byte-identical** re-derivation of the `0.17.6` patch (applied to a fresh 0.17.8
clone, the resulting `git diff` regenerated, and `diff` against the original returned zero — 12 files, no
drift, no hunk re-anchoring; the graft seam is untouched), so the transfer argument
...
`0.16.8-sqd.1` package** (now superseded by `0.17.8-sqd.1`; their assurance carries forward by the same
...
| `U-eth` | univ3 | eth | factory, logs, receipts | 10×500 (seeded) | **DONE — PASS under the documented pre-London `base_fee_per_gas` tolerance (§5.3).** UniswapV3 factory-stress on eth: **9 / 10 windows byte-identical** Portal-vs-RPC across all row families; one window (`[12453996, 12454496]`, all **pre-London** blocks) diverged **solely** on `blocks.base_fee_per_gas` — Portal SQL NULL vs RPC `0`, same block hash and every other column / all logs / transactions / receipts / traces byte-identical. Pre-London blocks carry **no `baseFeePerGas` field** (geth-verified absent on block 12453996), so **Portal's NULL is canonically correct and the RPC baseline's `0` is the outlier** — a benign representational diff, now a declared, hash-anchored, self-retiring tolerance (§5.3). Effective verdict: **PASS** modulo this documented benign class | `bash harness/validate/run-cell.sh U-eth` |
...
- **`U-eth`** is the UniswapV3 factory-stress cell (`10×500` seeded windows on eth). **DONE — PASS
  under the documented pre-London `base_fee_per_gas` tolerance (§5.3).** 9 / 10 windows were
  byte-identical Portal-vs-RPC across all row families; the one exception (`[12453996, 12454496]`, all
  pre-London) diverged **solely** on `blocks.base_fee_per_gas` — Portal SQL NULL vs RPC `0`, over an
  identical block hash. Pre-London blocks predate EIP-1559 and carry **no `baseFeePerGas` field**
  (geth-verified absent), so **Portal's NULL is canonically correct and the RPC `0` is the outlier**;
  the diff is now a declared, chain-scoped (eth-mainnet chain_id 1, like the access_list gap),
  hash-anchored, self-retiring benign class (§5.3). Effective verdict: **PASS** modulo that documented
  class.
...
than replacing it. (`0.17.8-sqd.1`, published 2026-08-18 and now `latest`, grafts the identical Portal layer;
`wiring/0.17.8.patch` is a **byte-identical** re-derivation of `wiring/0.17.6.patch`, which itself re-derives only `rpc/actions.ts` vs the `0.17.4` lineage, itself a **verbatim** copy of `wiring/0.17.3.patch` (a byte-identical verbatim copy back through `wiring/0.17.2.patch` / `wiring/0.17.1.patch` to `wiring/0.17.0.patch`);
...
in regions non-overlapping the wiring hunks), `0.17.2 → 0.17.3` is a single off-seam upstream memory fix
(`ponder-sh/ponder#2350` in `sync-realtime/index.ts`, at a line non-overlapping the wiring hunks), and
`0.17.3 → 0.17.4` is four off-seam upstream indexing-performance patch PRs (`ponder-sh/ponder#2353` / `#2356` / `#2355` / `#2352`,
all confined to ponder's indexing plane, with the three touched wiring files edited only above their non-overlapping wiring hunks), `0.17.4 → 0.17.6` re-derives only `rpc/actions.ts` (upstream moved address-chunking into `eth_getLogs`, off the graft seam), and `0.17.6 → 0.17.8` is a byte-identical re-derivation of the `0.17.6` patch (12 files, zero drift) — so this direct `0.17.0-sqd.1` fault evidence
carries forward unchanged to the current `0.17.8-sqd.1` build by patch + seam identity; it was not
re-run against `0.17.1-sqd.1`, `0.17.1-sqd.2`, `0.17.2-sqd.1`, `0.17.3-sqd.1`, `0.17.4-sqd.1`, `0.17.6-sqd.1`, or `0.17.8-sqd.1`.)
...
documented in `harness/soak-ab/ab-diff.mjs`; the three paid-matrix classes (the last three rows) — the
`block.size` derivation artifact (#76, merged in [#77](../../pull/77)), the `access_list` column gap
(#83/#32), and the pre-London `base_fee_per_gas` null-vs-0 representational diff (cell `U-eth`) — live
in both byte-diff paths (`harness/diff/diff.mjs` and `harness/validate/diff-batched.mjs`).
...
- The Portal layer's invariants (INV-1 … INV-27) hold under property-based tests **on every tracked
  upstream Ponder version** (`0.15.17`, `0.16.6`, `0.16.7`, `0.16.8`, `0.16.9`, `0.16.10`, `0.17.0`, `0.17.1`, `0.17.2`, `0.17.3`, `0.17.4`, `0.17.6`, `0.17.8` —
  each past `0.16.6` registered on a **seam-identity + full-suite** basis, *not* a fresh RPC byte-diff,
  §2; the full current suite is **416 tests / 24 files**, green on all of them), and every fix is backed
