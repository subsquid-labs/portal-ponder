...
import { assembleRange, createChunkData, rankTraces } from "./portal-assemble.js";
import { cmpTraceAddr } from "./portal-transform.js";
import { compileFetchSpec } from "./portal-filters.js";

setCheckMode("strict");

const header = (n: number) => ({ number: n, hash: "0x" + n.toString(16).padStart(64, "0"), parentHash: "0x0", timestamp: 1_700_000_000 + n });
const rawLog = (addr: string, tx: string) => ({ address: addr, topics: ["0xdead"], data: "0x", transactionHash: tx, transactionIndex: 0, logIndex: 0 });
const rawTx = (hash: string) => ({ transactionIndex: 0, hash, from: "0xfrom", to: "0xto", input: "0x", value: "0x0", nonce: 0, gas: "0x1", type: 0 });

const logFilter: any = { type: "log", chainId: 1, sourceId: "s", address: undefined, topic0: "0xdead", topic1: null, topic2: null, topic3: null, fromBlock: undefined, toBlock: undefined, hasTransactionReceipt: false, include: [] };
...
test("INV-2: assembleRange returns exactly the in-range logs (vs brute-force filter)", () => {
  fc.assert(
    fc.property(
      fc.array(fc.integer({ min: 0, max: 30 }), { minLength: 1, maxLength: 15 }), // block numbers with logs
      ...
      (blockNums, a, b) => {
        ...
        for (const bn of new Set(blockNums)) {
          ...
          cd.logs.set(bn, Array.from({ length: nLogs }, (_, i) => rawLog("0xVault", "0xtx" + bn + "_" + i)));
          if (bn >= lo && bn <= hi) for (let i = 0; i < nLogs; i++) model.push(bn);
        }
...
const mkTrace = (traceAddress: number[]) => ({ traceAddress, type: "call", subtraces: 0, action: { from: "0xfrom", to: "0xto", value: "0x0", gas: "0x1", input: "0x" + traceAddress.join(","), callType: "call" }, result: { gasUsed: "0x1", output: "0x" } });
...
test("INV-5: rankTraces sorts into pre-order DFS and assigns strictly-increasing full-tree ranks", () => {
  fc.assert(
    ...
    fc.property(fc.uniqueArray(arbTraceAddr, { comparator: (a, b) => cmpTraceAddr(a, b) === 0, maxLength: 8 }), (addrs) => {
      const ranked = rankTraces(addrs.map(mkTrace));
      const expectedOrder = [...addrs].sort(cmpTraceAddr);
      expect(ranked.map((r) => r.index)).toEqual(expectedOrder.map((_, i) => i)); // ranks are 0..n-1, strictly increasing
      // the k-th ranked frame is the k-th trace in cmpTraceAddr order (verified via the encoded input)
      expect(ranked.map((r) => (r.frame as any).input)).toEqual(expectedOrder.map((ta) => "0x" + ta.join(",")));
    }),
  );
});
...
test("closest includes a trace-only block above the highest log block", () => {
  ...
  const traceFilter: any = { type: "trace", chainId: 1, sourceId: "t", fromAddress: undefined, toAddress: TARGET, functionSelector: undefined, callType: undefined, includeReverted: false, fromBlock: 0, toBlock: 1000, hasTransactionReceipt: false, include: [] };
  const cd = createChunkData();
  cd.headers.set(10, header(10));
  cd.logs.set(10, [rawLog("0xV", "0xtx10")]); // a log block at 10
  cd.traceBlocks.set(20, { header: header(20), traces: [{ transactionIndex: 0, traceAddress: [], type: "call", subtraces: 0, action: { from: "0xother", to: TARGET, value: "0x0", gas: "0x1", input: "0x", callType: "call" }, result: { gasUsed: "0x1", output: "0x" } }], txs: [rawTx("0xtx20")] });
  const spec = compileFetchSpec([{ filter: logFilter }, { filter: traceFilter }], new Map());
