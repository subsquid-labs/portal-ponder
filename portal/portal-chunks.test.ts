  ...
  fc.assert(
    fc.property(
      ...
      (cb, a, b) => {
        ...
        const startIdx = idxOf(lo, cb), endIdx = idxOf(hi, cb);
        const ranges: [number, number][] = [];
        for (let i = startIdx; i <= endIdx; i++) ranges.push(chunkRange(i, cb, 0, INF));
...
test("scaleChunkBlocks: density scaling ×head/25M, capped 25M", () => {
  ...
  fc.assert(fc.property(fc.integer({ min: 0, max: 2_000_000_000 }), (head) => {
  ...
  fc.assert(
    fc.property(
      ...
      (endIdx, readahead, saturated) => {
        ...
        if (saturated) expect(plan.length).toBeLessThanOrEqual(1); // depth-1 always admitted
        // strictly increasing, starting at endIdx+1
        plan.forEach((idx, i) => expect(idx).toBe(endIdx + 1 + i));
      },
    ),
  );
  // never past raEnd
...
test("eviction plan: only chunks whose whole span is behind the cursor", () => {
  ...
  fc.assert(fc.property(fc.array(fc.integer({ min: 0, max: 100 })), fc.integer({ min: 0, max: 100_000 }), (idxs, start) => {
    for (const i of evictionPlan(idxs, 1000, start)) expect((i + 1) * 1000).toBeLessThanOrEqual(start);
  }));
});
