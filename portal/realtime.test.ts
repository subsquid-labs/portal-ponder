...
test("portalRpc: builds the portal RPC URL and attaches the x-api-key header", () => {
  const t = portalRpc(42161, "prt_test");
  ...
  const t2: any = portalRpc(1, "k", { url: "http://local/rpc" })({} as any);
  expect(t2.value?.url).toBe("http://local/rpc/1");
});

test("portalRealtime / rpcRealtime return composed viem transports", () => {
  expect(typeof portalRealtime(1, "k", ["http://a", "http://b"])).toBe("function");
