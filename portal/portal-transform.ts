...
// assemble / shell boundaries so those exported signatures avoid `any`; the transforms below stay `any`
// (they are the intentionally-unchanged Sync* cast boundary).
export type RawLog = Record<string, unknown> & { address?: string; topics?: string[]; data?: string; transactionHash?: string; transactionIndex?: number; logIndex?: number };
export type RawTx = Record<string, unknown> & { transactionIndex?: number; hash?: string; from?: string; to?: string | null; type?: number | string };
export type RawTrace = Record<string, unknown> & { transactionIndex?: number | null; traceAddress?: number[]; type?: string };
export type RawBlock = { header: RawHeader; logs?: RawLog[]; transactions?: RawTx[]; traces?: RawTrace[] };
