...
const arbEvent: fc.Arbitrary<GateEvent> = fc.oneof(
  ...
  fc
    ...
    .map<GateEvent>((n) => ({ type: 'addRows', n })),
  fc
    .integer({ min: 0, max: 500 })
    .map<GateEvent>((n) => ({ type: 'freeRows', n })),
);
...
test('createGate: FIFO admission, no starvation while capacity exists', async () => {
  const cfg = loadPortalConfig({
    PORTAL_START_CONCURRENCY: '2',
    PORTAL_MIN_CONCURRENCY: '1',
    PORTAL_MAX_CONCURRENCY: '4',
  });
  const gate = createGate(cfg);
  const order: number[] = [];
  const ps = Array.from({ length: 5 }, (_, i) =>
    gate.acquire().then(() => order.push(i)),
  );
  await Promise.resolve(); // flush microtasks: first 2 admitted (start=2)
  expect(gate.snapshot().active).toBe(2);
  gate.release();
  gate.release();
  gate.release(); // admit the remaining 3 in FIFO order
  await Promise.all(ps);
  expect(order).toEqual([0, 1, 2, 3, 4]); // strict FIFO — every waiter served
});

test('createGate: an AIMD ramp PUMPS parked waiters (onOk raises the limit and admits)', async () => {
  const cfg = loadPortalConfig({
    PORTAL_START_CONCURRENCY: '2',
    PORTAL_MIN_CONCURRENCY: '1',
    PORTAL_MAX_CONCURRENCY: '4',
  ...
  const gate = createGate(cfg);
  await gate.acquire();
  await gate.acquire(); // active = 2 = limit
  let admitted = false;
  const waiter = gate.acquire().then(() => {
    admitted = true;
  });
  await Promise.resolve();
  expect(admitted).toBe(false); // parked at the limit
  for (let i = 0; i < 8; i++) gate.onOk(); // 8 clean → limit 2 → 4, and the pump must run
  await waiter;
  expect(admitted).toBe(true); // the ramp itself admitted the waiter (no release needed)
});
