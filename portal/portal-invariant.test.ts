...
import { getCheckMode, invariant, invariantStrict, setCheckMode } from "./portal-invariant.js";
...
test("mode 'on': O(1) invariant throws InvariantViolation carrying id + context", () => {
  ...
  let thrown: unknown;
  try {
    invariant("INV-8", false, "limit out of range", () => ({ limit: 99, max: 48 }));
  } catch (e) {
    thrown = e;
  }
...
test("mode 'off': all checks disabled", () => {
  ...
  expect(() => invariantStrict("INV-5", () => false, "should be skipped")).not.toThrow();
});

test("O(n) invariantStrict runs ONLY in strict mode (predicate not even evaluated otherwise)", () => {
  let evaluated = 0;
  const pred = () => { evaluated++; return false; };
  ...
  setCheckMode("strict");
  expect(() => invariantStrict("INV-5", pred, "strict only")).toThrow(InvariantViolation);
