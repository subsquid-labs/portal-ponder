...
import { DROPPABLE_FIELDS, MAX_RAW_QUERY_SIZE, PORTAL_MAX_ADDRESSES, type PortalQuery } from "./portal-filters.js";
...
export async function* ndjsonLines(
  ...
  for (;;) {
    ...
    if (done) break;
    if (value) { onBytes?.(value.byteLength); buf += dec.decode(value, { stream: true }); }
    let nl: number;
...
const colToFieldKey = (col: string, table: string): string => {
  ...
  const field = COL_SPECIAL[col] ?? col.replace(/_([a-z0-9])/g, (_, c: string) => c.toUpperCase());
  return `${key}.${field}`;
};

const stripFields = (q: PortalQuery, dropped: Set<string>): PortalQuery => {
  if (dropped.size === 0 || !q.fields) return q;
  const fields: Record<string, Record<string, boolean>> = JSON.parse(JSON.stringify(q.fields));
  for (const tf of dropped) { const i = tf.indexOf("."); const t = tf.slice(0, i), f = tf.slice(i + 1); const tbl = fields[t]; if (tbl) delete tbl[f]; }
  return { ...q, fields };
};
...
export function createPortalClient(deps: PortalClientDeps): PortalClient {
  ...
  async function fetchBatch(body: string, cursor: number): Promise<{ blocks: RawBlock[]; last: number } | "done"> {
    ...
    if (body.length > MAX_RAW_QUERY_SIZE) {
      ...
      const nLog = (q.logs ?? []).reduce((s: number, r: { address?: unknown[] }) => s + (r.address?.length ?? 0), 0);
      const nTx = (q.transactions ?? []).reduce((s: number, r: { from?: unknown[]; to?: unknown[] }) => s + (r.from?.length ?? 0) + (r.to?.length ?? 0), 0);
      throw new Error(
    ...
    const tAcq = Date.now(); await gate.acquire(); stats.gateWaitMs += Date.now() - tAcq; // gate-wait = concurrency back-pressure
    const tFetch = Date.now();
    stats.inflight++; stats.maxInflight = Math.max(stats.maxInflight, stats.inflight);
    try {
      const res = await fetchImpl(`${portalUrl}/finalized-stream?buffer_size=${bufferSize}`, { method: "POST", headers, body });
      stats.http++;
      if (res.status === 204) { gate.onOk(); return "done"; }
      ...
      if (res.status >= 500 || res.status === 429 || res.status === 409) {
        ...
        gate.onThrottle();
        throw new PortalThrottleError(res.status, Number.isFinite(ra) ? ra * 1000 : undefined);
      }
      if (!res.ok) {
        ...
        if (m) throw new PortalSchemaFieldError(colToFieldKey(m[1]!, m[2]!), TABLE_TO_KEY[m[2]!] ?? m[2]!, m[1]!);
        ...
        if (res.status === 400 && /query is too large/i.test(text)) throw new PortalQueryTooLargeError(cursor);
        throw new PortalHttpError(res.status, cursor, text);
      }
      const blocks: RawBlock[] = [];
      let last = cursor;
      for await (const line of ndjsonLines(res.body!, (n) => { stats.bytes += n; })) {
        const b = JSON.parse(line) as RawBlock;
        blocks.push(b);
        if (typeof b.header?.number === "number" && b.header.number > last) last = b.header.number;
      }
      ...
      throw err;
    } finally { stats.fetchMs += Date.now() - tFetch; gate.release(); stats.inflight--; }
  }

  async function* stream(query: PortalQuery, from: number, to: number, opts?: StreamOpts): AsyncGenerator<RawBlock[]> {
      ...
      while (batch === undefined) {
        ...
        try {
          ...
          if (err instanceof PortalDatasetStartError) {
            ...
            if (err.startsAt > cursor) { cursor = err.startsAt; continue; } // skip the missing prefix
          ...
          if (err instanceof PortalSchemaFieldError) {
            ...
            if (!DROPPABLE_FIELDS.has(err.fieldKey)) neededMissing?.add(`${err.fieldKey} (${err.tag})`);
            else logDebug(`Portal ${chainName} [${from},${to}]: dataset can't serve '${err.tag}' → skipping non-load-bearing field ${err.fieldKey}`);
            continue;
          }
          if (!isTransientError(err) || attempt++ >= 10) throw err;
          stats.errors++; stats.retries++;
          const wait = err instanceof PortalThrottleError && err.retryAfterMs !== undefined ? Math.min(err.retryAfterMs, 30_000) : Math.min(500 * 2 ** attempt, 30_000);
          await sleep(wait);
        }
      }
  ...
  const finalizedHead = async (): Promise<number | undefined> => {
    try {
      const h = await fetchImpl(`${portalUrl}/finalized-head`, { headers }).then((r) => r.json());
      if (typeof h?.number === "number") return h.number;
    } catch { /* head unknown → caller stays conservative */ }
    return undefined;
  };
