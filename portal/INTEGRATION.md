...
On a fresh process the first fetches slow-start instead of immediately streaming an entire chunk. `PORTAL_WARMUP_BLOCKS` (default 25,000) bounds both the fetch span and the discovery lookahead; each has its own per-process quantum and each converges geometrically after successful work until the request pattern is the same as the legacy full-chunk/full-lookahead path. Interval-serving fetches also trim the low edge to the requested interval, so a resumed process does not re-stream the already-committed prefix of a large chunk before its first durable commit. Set `PORTAL_WARMUP_BLOCKS=0` to disable this warmup path and restore the legacy fetch shape.
...
| `PORTAL_WARMUP_BLOCKS` | `25000` | Initial process warmup quantum for both data fetches and discovery lookahead; each doubles geometrically after successful work. Set `0` to disable warmup and use the legacy full-chunk/full-lookahead shape. |
...
| `PORTAL_ALLOW_MISSING_ACCESS_LIST` | unset | Set to exactly `1` to relax the fail-loud guard on typed-envelope transactions (EIP-2930/1559/4844/7702 → `0x1`/`0x2`/`0x3`/`0x4`) whose spec-required `accessList` is missing. Default OFF: such a transaction is a loud error (silent NULL data loss otherwise, #27/#110). Only enable for RPCs that *legitimately* omit `accessList` (e.g. some proxies such as SQD's zkSync proxy); the missing value is then stored as an **honest NULL** (never a fabricated `[]`), and a present `accessList` is always preserved. Parsed strictly — only `1` enables it; `0`/`false`/`no`/empty keep the guard fail-loud. Enabling it logs a one-time startup warning. |
