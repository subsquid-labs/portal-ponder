...
import type { Address, Factory, LightBlock, LogFilter } from "@/internal/types.js";
import {
...
const DEPOSIT = "0xdcbc1c05240f31ff3ad067ef1ee35ce4997762752e3a095284754544f4c709d7";
...
test("lightToLightBlock: Portal decimal number/timestamp → ponder hex, hashes passthrough", () => {
  const lb = lightToLightBlock({ number: 101, hash: "0xabc", parentHash: "0xdef", timestamp: 1712 });
  expect(lb).toEqual({ number: "0x65", hash: "0xabc", parentHash: "0xdef", timestamp: "0x6b0" });
});
...
test("discoverChildAddresses: ignores non-matching logs (wrong selector / wrong factory address)", () => {
  const factory = eulerFactory();
  const wrongSelector = proxyLog("0x2222222222222222222222222222222222222222", { topics: [DEPOSIT, topicAddr("0x2222222222222222222222222222222222222222")] });
  const wrongAddress = proxyLog("0x3333333333333333333333333333333333333333", { address: "0xdeadbeef00000000000000000000000000000000" });
  const out = discoverChildAddresses([wrongSelector, wrongAddress] as any, [factory]);
...
test("discoverChildAddresses: multiple children in one block", () => {
  ...
  const out = discoverChildAddresses([proxyLog(a), proxyLog(b)] as any, [factory]);
...
test("toRealtimeSyncEvent: block → log-only BlockWithEventData (no txs/receipts/traces, childAddresses, no blockCallback)", () => {
  const factory = eulerFactory();
  const childAddresses = new Map([[factory, new Set<Address>(["0xchild" as Address])]]);
  const block: PortalRealtimeEvent = {
    type: "block",
    block: { number: "0x65", hash: "0xh", parentHash: "0xp", timestamp: "0x1" } as any,
    logs: [{ address: "0xchild" } as any],
    hasMatchedFilter: true,
  };
  const ev = toRealtimeSyncEvent(block, childAddresses) as Extract<ReturnType<typeof toRealtimeSyncEvent>, { type: "block" }>;
...
test("toRealtimeSyncEvent: reorg / finalize pass through with hex LightBlocks", () => {
  const reorg = toRealtimeSyncEvent(
    { type: "reorg", block: { number: 10, hash: "a", parentHash: "z", timestamp: 10 }, reorgedBlocks: [{ number: 11, hash: "b", parentHash: "a", timestamp: 11 }] },
    new Map(),
  );
  expect(reorg).toEqual({ type: "reorg", block: { number: "0xa", hash: "a", parentHash: "z", timestamp: "0xa" }, reorgedBlocks: [{ number: "0xb", hash: "b", parentHash: "a", timestamp: "0xb" }] });
  const finalize = toRealtimeSyncEvent({ type: "finalize", block: { number: 12, hash: "c", parentHash: "b", timestamp: 12 } }, new Map());
  expect(finalize).toEqual({ type: "finalize", block: { number: "0xc", hash: "c", parentHash: "b", timestamp: "0xc" } });
});
...
test("buildPortalLogRequests: factory-address log filter → known children, plus a ProxyCreated discovery request", () => {
  const childAddresses = new Map([["euler-factory", new Map<Address, number>([["0xvault1" as Address, 100], ["0xvault2" as Address, 101]])]]);
  const reqs = buildPortalLogRequests([{ filter: logFilter() }], childAddresses);
...
test("buildPortalLogRequests: a factory filter with no known children yet still emits the discovery request", () => {
  ...
  expect(reqs.some((r) => r.topic0?.includes(PROXY_CREATED) && r.address?.[0] === FACTORY_ADDR)).toBe(true);
});
...
test("clampFinalizedToPortalHead: A-path passthrough — flag off never touches the finalized block or the network", async () => {
  delete process.env.PORTAL_REALTIME;
  let called = false;
  const fetchImpl = (async () => { called = true; return { json: async () => ({ number: 1 }) }; }) as any;
  const finalized = { number: "0x3e8", hash: "0xh", parentHash: "0xp", timestamp: "0x1" } as LightBlock;
  const out = await clampFinalizedToPortalHead({ chain: { portal: "http://p", name: "c" } as any, rpc: {} as any, finalizedBlock: finalized, fetchImpl });
...
test("clampFinalizedToPortalHead: Portal at/ahead of RPC finalized → no clamp (never RAISES the boundary)", async () => {
  process.env.PORTAL_REALTIME = "stream";
  const fetchImpl = (async () => ({ json: async () => ({ number: 5000 }) })) as any; // portal head ≥ finalized 1000
  const finalized = { number: "0x3e8", hash: "0xh", parentHash: "0xp", timestamp: "0x1" } as LightBlock;
  const out = await clampFinalizedToPortalHead({ chain: { portal: "http://p", name: "c" } as any, rpc: {} as any, finalizedBlock: finalized, fetchImpl });
...
test("clampFinalizedToPortalHead: Portal head unknown (probe fails) → conservative passthrough", async () => {
  process.env.PORTAL_REALTIME = "stream";
  const fetchImpl = (async () => { throw new Error("down"); }) as any;
  const finalized = { number: "0x3e8", hash: "0xh", parentHash: "0xp", timestamp: "0x1" } as LightBlock;
  const out = await clampFinalizedToPortalHead({ chain: { portal: "http://p", name: "c" } as any, rpc: {} as any, finalizedBlock: finalized, fetchImpl });
  expect(out).toBe(finalized);
});
...
test("clampFinalizedToPortalHead: Portal head BELOW RPC finalized → refetch the block at the Portal head", async () => {
  process.env.PORTAL_REALTIME = "stream";
  const fetchImpl = (async () => ({ json: async () => ({ number: 900 }) })) as any; // portal head 900 < finalized 1000
  let requested: any;
  const fullBlock = {
    number: "0x384", hash: "0xportalhead", parentHash: "0xpp", timestamp: "0x2",
    logsBloom: `0x${"0".repeat(512)}`, sha3Uncles: "0x0", miner: "0x0", stateRoot: "0x0",
    transactionsRoot: "0x0", receiptsRoot: "0x0", gasUsed: "0x0", gasLimit: "0x0",
    extraData: "0x", nonce: "0x0", mixHash: "0x0", difficulty: "0x0", size: "0x0", transactions: [],
  };
  const rpc = { request: async (req: any) => { requested = req; return fullBlock; } } as any;
  const finalized = { number: "0x3e8", hash: "0xh", parentHash: "0xp", timestamp: "0x1" } as LightBlock;
  const out = await clampFinalizedToPortalHead({ chain: { portal: "http://p", name: "c" } as any, rpc, finalizedBlock: finalized, fetchImpl });
...
function mockPortal(batches: any[], finalizedHead: number) {
  ...
  return (async (url: string) => {
    if (url.endsWith("/finalized-head")) return { json: async () => ({ number: finalizedHead }) };
...
test("getPortalRealtimeEventGenerator: streams block events, discovers a new child, and updates the running childAddresses; terminates at endBlock", async () => {
  ...
  const batches = [
    ...
    { header: { number: 101, hash: "h101", parentHash: "h100", timestamp: 1012 }, logs: [proxyLog(child, { blockNumber: 101 })] },
  ];
  const childAddresses = new Map<string, Map<Address, number>>([["euler-factory", new Map()]]);
  const events: any[] = [];
  for await (const { event } of getPortalRealtimeEventGenerator({
    ...
    syncProgress: { finalized: { number: "0x63", hash: "h99", parentHash: "h98", timestamp: "0x1" } as any, end: { number: "0x65" } as any },
    childAddresses,
...
test("getPortalRealtimeEventGenerator: emits a monotonic finalize (above the startup finalized) from the head poll", async () => {
  ...
  const batches = [
    { header: { number: 100, hash: "h100", parentHash: "h99", timestamp: 1000 }, logs: [] },
    { header: { number: 101, hash: "h101", parentHash: "h100", timestamp: 1012 }, logs: [] },
  ];
  const events: any[] = [];
  for await (const { event } of getPortalRealtimeEventGenerator({
    common: { logger: { info() {}, debug() {}, warn() {}, trace() {} } } as any,
