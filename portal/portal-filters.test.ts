...
const logFilter = (over: Partial<LogFilter> = {}): LogFilter => ({
  type: "log", chainId: 1, sourceId: "s", address: undefined, topic0: "0x00" as any,
  topic1: null, topic2: null, topic3: null, fromBlock: undefined, toBlock: undefined,
  hasTransactionReceipt: false, include: [], ...over,
}) as LogFilter;

// ── INV-11: merge equivalence vs a model log-matcher ──────────────────────────────────────────────

type ModelLog = { address: string; topic0: string; topic1: string };
...
const matchesAny = (reqs: PortalLogRequest[], l: ModelLog): boolean => reqs.some((r) => matchesReq(r, l));
...
const arbReq: fc.Arbitrary<PortalLogRequest> = fc.record({
  address: fc.option(fc.subarray(addrPool, { minLength: 1 }).map((a) => [...a].sort()), { nil: undefined }),
  topic0: fc.option(fc.subarray(t0Pool, { minLength: 1 }), { nil: undefined }),
  topic1: fc.option(fc.constantFrom(...t1Pool).map((t) => [t]), { nil: undefined }),
});
...
test("INV-11: mergeLogRequests matches exactly the same logs as the originals", () => {
  fc.assert(
    fc.property(fc.array(arbReq, { maxLength: 8 }), fc.array(arbLog, { maxLength: 20 }), (reqs, logs) => {
      const merged = mergeLogRequests(reqs);
      for (const l of logs) expect(matchesAny(merged, l)).toBe(matchesAny(reqs, l));
    }),
  );
});
...
test("INV-11: N same-address event filters collapse to ONE request, topic0 unioned", () => {
  ...
  const reqs = Array.from({ length: 6 }, (_, i) => ({ address: [ADDR], topic0: ["0x" + (i + 1).toString(16)] }));
  const merged = mergeLogRequests(reqs);
  expect(merged).toHaveLength(1);
  expect(new Set(merged[0]!.topic0)).toEqual(new Set(reqs.map((r) => r.topic0![0])));
});

test("INV-11: an undefined topic0 absorbs all (kept broadest)", () => {
  const merged = mergeLogRequests([{ address: ["0xa"], topic0: ["0x1"] }, { address: ["0xa"] }]);
  expect(merged).toHaveLength(1);
...
test("logRequestsFor: plain-address batches by PORTAL_MAX_ADDRESSES", () => {
  const addrs = Array.from({ length: PORTAL_MAX_ADDRESSES * 2 + 5 }, (_, i) => ("0x" + i.toString(16).padStart(40, "0")) as Address);
...
test("logRequestsFor: factory filter with zero children → [] (never match-all)", () => {
  const factory: any = { id: "f", type: "log", address: "0xF", eventSelector: "0xsel", childAddressLocation: "topic1" };
...
test("logRequestsFor: factory filter expands to known children", () => {
  const factory: any = { id: "f", type: "log", address: "0xF", eventSelector: "0xsel", childAddressLocation: "topic1" };
  const childAddresses: ChildAddresses = new Map([["f", new Map([["0xchild1" as Address, 10], ["0xchild2" as Address, 20]])]]);
...
test("INV-5 support: the trace request carries NO trace filter (fetch-all)", () => {
  const trace: any = { type: "trace", chainId: 1, sourceId: "t", fromAddress: undefined, toAddress: "0xdead", functionSelector: undefined, callType: undefined, includeReverted: false, fromBlock: 0, toBlock: 100, hasTransactionReceipt: false, include: [] };
...
test("txQuery: account from/to filters pushed server-side; match-all skipped", () => {
  const txf: any = { type: "transaction", chainId: 1, sourceId: "x", fromAddress: "0xFROM", toAddress: undefined, includeReverted: false, fromBlock: 0, toBlock: 100, hasTransactionReceipt: true, include: [] };
