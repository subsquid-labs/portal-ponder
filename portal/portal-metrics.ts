...
export type PortalStats = {
  dataChunks: number; discChunks: number; http: number; logs: number; errors: number; retries: number;
  bytes: number; cacheHits: number; inflight: number; maxInflight: number; blocks: number; txs: number;
  receipts: number; traces: number; rpcFallback: number; gateWaitMs: number; fetchMs: number; transformMs: number;
};

export const createStats = (): PortalStats => ({
  dataChunks: 0, discChunks: 0, http: 0, logs: 0, errors: 0, retries: 0, bytes: 0, cacheHits: 0,
  inflight: 0, maxInflight: 0, blocks: 0, txs: 0, receipts: 0, traces: 0, rpcFallback: 0,
  gateWaitMs: 0, fetchMs: 0, transformMs: 0,
});
...
export function writeMetrics(args: {
  ...
  const { metricsFile, chain, stats, chunkBlocks, portalHead, gate, startTime } = args;
  if (!metricsFile) return;
  try {
    writeFileSync(`${metricsFile}.${chain.id}`, JSON.stringify({
      chain: chain.name, chainId: chain.id, wallMs: startTime ? Date.now() - startTime : 0,
...
export function startGateLog(gate: Gate, enabled: boolean, intervalMs = 20_000): void {
  if (!enabled || ticker !== undefined) return;
  ticker = setInterval(() => {
    const s = gate.snapshot();
    console.log(`[portalGate] concurrency_limit=${s.limit} active=${s.active} buffered_rows=${s.rows}`);
  }, intervalMs);
  ticker.unref();
}
