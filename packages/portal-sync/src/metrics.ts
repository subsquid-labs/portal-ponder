...
export type StatusKey = "200" | "204" | "409" | "429" | "500" | "503" | "529" | "other";
...
const emptyStatus = (): Record<StatusKey, number> => ({
  "200": 0, "204": 0, "409": 0, "429": 0, "500": 0, "503": 0, "529": 0, other: 0,
});

const statusKey = (status: number): StatusKey => {
  const k = String(status) as StatusKey;
  return (["200", "204", "409", "429", "500", "503", "529"] as StatusKey[]).includes(k) ? k : "other";
};
...
export class PortalMetrics {
  ...
  private ds(dataset: string): DatasetMetrics {
    ...
    if (d === undefined) {
      d = {
        dataset, logicalStreams: 0, httpRequests: 0, status: emptyStatus(),
        blocksRequested: 0, forwardProgressBlocks: 0, blocksEmitted: 0,
        logs: 0, transactions: 0, traces: 0, bytes: 0, streamMillis: 0,
        retries: 0, retryAfterWaitMillis: 0, firstBlock: undefined, lastBlock: undefined,
      };
  ...
  onLogicalStream(dataset: string, fromBlock: number, toBlock: number): void {
    ...
    if (d.firstBlock === undefined || fromBlock < d.firstBlock) d.firstBlock = fromBlock;
  }

  onHttpResponse(dataset: string, r: {
    status: number; bytes: number; durationMs: number;
    blocks?: number; logs?: number; transactions?: number; traces?: number;
    lastBlock?: number;
  }): void {
    const d = this.ds(dataset);
    d.httpRequests++;
    ...
    if (r.lastBlock !== undefined) {
      if (d.lastBlock === undefined || r.lastBlock > d.lastBlock) d.lastBlock = r.lastBlock;
      if (d.firstBlock !== undefined) {
        d.forwardProgressBlocks = Math.max(d.forwardProgressBlocks, r.lastBlock - d.firstBlock + 1);
      }
    }
  }
  ...
  snapshot(now = Date.now()) {
    ...
    const perDataset = [...this.datasets.values()].map((d) => {
      const clientFacingErrors = d.status["503"] + d.status["529"] + d.status["500"];
      const httpPerLogicalStream = d.logicalStreams ? d.httpRequests / d.logicalStreams : 0;
      ...
      return {
        ...
        streamsPer1000Blocks: blocks ? round((d.httpRequests / blocks) * 1000, 3) : 0,
        forwardProgressBlocks: d.forwardProgressBlocks,
        blocksPerSec: d.streamMillis ? Math.round((d.forwardProgressBlocks / d.streamMillis) * 1000) : 0,
        ...
        range: d.firstBlock !== undefined ? [d.firstBlock, d.lastBlock] : undefined,
      };
    });
    const totals = perDataset.reduce(
      (a, p) => ({
        ...
        forwardProgressBlocks: a.forwardProgressBlocks + p.forwardProgressBlocks,
        logs: a.logs + p.logs,
      ...
      { logicalStreams: 0, httpRequests: 0, forwardProgressBlocks: 0, logs: 0, traces: 0, mib: 0, clientFacingErrors: 0, cuEstimate: 0 },
    );
    return {
  ...
  prometheus(): string {
    ...
    g("portal_logical_streams_total", "High-level Portal stream() calls (one per fork interval-query)", "counter");
    g("portal_http_requests_total", "Actual Portal HTTP POSTs incl. continuations, by status", "counter");
    g("portal_blocks_scanned_total", "Forward-progress blocks scanned per dataset", "counter");
    ...
    g("portal_client_facing_errors_total", "503/529/500 surfaced to the client", "counter");
    g("portal_cu_estimate", "Estimated compute units (blocks/assumedChunkBlocks)", "gauge");
    for (const d of this.datasets.values()) {
      ...
      lines.push(`portal_blocks_scanned_total{${l}} ${d.forwardProgressBlocks}`);
      lines.push(`portal_logs_total{${l}} ${d.logs}`);
      lines.push(`portal_bytes_total{${l}} ${d.bytes}`);
      lines.push(`portal_client_facing_errors_total{${l}} ${d.status["503"] + d.status["529"] + d.status["500"]}`);
      lines.push(`portal_cu_estimate{${l}} ${round((d.forwardProgressBlocks || d.blocksRequested) / this.assumedChunkBlocks, 2)}`);
    }
    return lines.join("\n") + "\n";
  }
}
