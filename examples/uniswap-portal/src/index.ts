...
ponder.on("Every1000:block", async ({ event, context }) => {
  await context.db
    .insert(blockTick)
    .values({ number: event.block.number, timestamp: event.block.timestamp });
});
...
ponder.on("UsdcWethPool:Swap", async ({ event, context }) => {
  await context.db.insert(swap).values({
    ...
    receiptStatus: event.transactionReceipt
      ? event.transactionReceipt.status === "success"
        ? 1
        : 0
      : null,
    blockNumber: event.block.number,
...
const onRouterCall =
  ...
  async ({ event, context }: any) => {
    await context.db.insert(routerCall).values({
      ...
      to: event.trace.to ?? null,
      blockNumber: event.block.number,
    });
  };
ponder.on(
  "UniswapV2Router02.swapExactTokensForTokens()",
  onRouterCall("swapExactTokensForTokens"),
);
ponder.on(
  "UniswapV2Router02.swapExactETHForTokens()",
  onRouterCall("swapExactETHForTokens"),
);
ponder.on(
  "UniswapV2Router02.swapExactTokensForETH()",
  onRouterCall("swapExactTokensForETH"),
);
