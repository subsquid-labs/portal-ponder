...
`@subsquid/ponder` changes exactly one thing about Ponder: where the **historical** backfill gets its data. Realtime, reorg handling, handlers, schema, and the `ponder` CLI are untouched. Yet that one change makes a full-history resync several times faster — and the reason is worth understanding, because it comes from two layers, not one.
...
Because the Portal serves only finalized data, the split between the two syncs is clean: **historical owns `[start, finalized]`; realtime owns `(finalized, tip]`.**

**Realtime stays on your RPC by default.** When the backfill reaches the finalized head, Ponder hands off to its own realtime sync over `rpc`, with its own reorg handling — byte-for-byte the stock path. The Portal is never in the realtime path unless you opt in, which is why `rpc` stays configured (it also serves chain setup and `readContract`).
...
**Portal-native realtime** is an opt-in, experimental alternative. Set `PORTAL_REALTIME=stream` to serve the tip from the Portal's fork-aware `/stream` (hot blocks) instead of RPC. In this mode `clampFinalizedToPortalHead` lowers Ponder's finalized boundary to the Portal head, so historical stops exactly there and realtime streams `(portal-head, tip]`; reorgs are reconciled from the stream's parent-hash chain and surfaced as Ponder's own `reorg` / `finalize` events, so downstream indexing and checkpointing are unchanged. With the flag off, this path is inert.
